...
const rm = (path, opts = {}) => {
  ...
  if (opts.f) {
    ...
    var changesToRm = Utils.intersection(Diff.addedOrModifiedFiles(), filesToRm);
...
const commit = (opts) => {
  ...
  if (
    ...
    if (Merge.isMergeInProgress() && conflictedPaths.length > 0) {
      ...
      const m = Merge.isMergeInProgress() ? Files.read(Files.enkelgitPath('MERGE_MSG')) : opts.m;
...
const checkout = (ref, _) => {
  ...
  } else if (ref === Refs.headBranchName() || ref === Files.read(Files.enkelgitPath('HEAD'))) {
...
const clone = (remotePath, targetPath, opts = {}) => {
  ...
  } else if (!fs.existsSync(remotePath) || !Utils.onRemote(remotePath)(Files.inRepo)) {
